    ...
    <div class="container">
        ...
        <form action="{{ route('cars.update', $car) }}" method="POST">
            ...
            <div class="mb-3"><label class="form-label">Ціна за день</label>
                <input type="number" step="0.01" name="price_per_day" class="form-control" value="{{ old('price_per_day', $car->price_per_day) }}" required>
            </div>
            <div class="mb-3"><label class="form-label">Категорія</label>
                <select name="car_category_id" class="form-select" required>
                    <option value="">– Оберіть –</option>
                    @foreach($categories as $c)
                        <option value="{{ $c->id }}"{{ old('car_category_id', $car->car_category_id)==$c->id?' selected':'' }}>{{ $c->name }}</option>
                    @endforeach
                </select>
            </div>
